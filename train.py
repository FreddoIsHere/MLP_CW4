...
parser.add_argument('--num_episodes', nargs="?", type=int, default=500, help='number of episodes')
parser.add_argument('--max_steps', nargs="?", type=int, default=30, help='number of steps')
...
plt.plot(rewards)
average = np.convolve(rewards, (1/5)*np.ones(5), mode='valid')
plt.plot(average)
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.show()
...
plt.plot(losses)
average = np.convolve(losses, (1/5)*np.ones(5), mode='valid')
plt.plot(average)
plt.xlabel("Episode")
plt.ylabel("Loss")
plt.show()